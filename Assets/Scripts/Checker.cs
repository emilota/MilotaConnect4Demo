...
namespace MilotaConnect4Demo
{
    public class Checker
    {
            ...
            Action<Checker> onHitTarget)
        {
            ...
            GameObject goBoardRoot = this.CheckerManager.Board.App.GameSceneMB.GOBoardRoot;
            GameObject prefab = this.CheckerManager.Board.App.GameSceneMB.PrefabQuestionMark;
            switch (this.WhichPlayerChecker)
            {
                ...
                case WhichPlayer.PLAYER_1_HUMAN:
                    {
                        prefab = this.CheckerManager.Board.App.GameSceneMB.PrefabPlayer1Checker;
                        break;
                    }
                case WhichPlayer.PLAYER_2_AI:
                    {
                        prefab = this.CheckerManager.Board.App.GameSceneMB.PrefabPlayer2Checker;
                        break;
                    }
                default:
                    {
        ...
        public void OnCheckerFixedUpdate()
        {
            if ((this.CheckerManager != null) && (!this.AllDone))
            {
                ...
                float delta = (
                        ...
                        board.App.GameSceneMB.CheckerDropScale);
                if (distance <= delta)
                {
                    // hit target!
                ...
                else
                {
                    ...
                    this.PosCurrent += (board.UIMetrics.DY * board.App.GameSceneMB.CheckerDropScale);
                    if (this.GOChecker != null)
                        this.GOChecker.transform.position = this.PosCurrent;
                }
            }
